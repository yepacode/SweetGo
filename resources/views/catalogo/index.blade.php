...
@section('content')
    <div class="mb-6">
        <h2 class="text-xl font-semibold text-gray-800">Catálogo público</h2>
        <p class="text-sm text-gray-500">Comparte tu catálogo por WhatsApp o redes. Tus clientes ven productos y precios sin registrarse.</p>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="bg-white rounded-xl border border-sweetgo-pink-light p-6 h-fit">
            ...
            <form method="POST" action="{{ route('catalogo.enlaces.crear') }}" class="space-y-3">
                @csrf
                ...
                <div x-data="{ copiado: false }" class="bg-white rounded-xl border border-sweetgo-pink-light p-5">
                    ...
                    <div class="flex flex-wrap items-center gap-2 mt-4">
                        ...
                        <div class="ml-auto flex items-center gap-2">
                            <form method="POST" action="{{ route('catalogo.enlaces.toggle', $enlace) }}">
                                @csrf @method('PATCH')
                                <button class="px-3 py-1.5 rounded-lg text-xs text-gray-500 hover:bg-gray-50">{{ $enlace->activo ? 'Desactivar' : 'Activar' }}</button>
                            </form>
                            <form method="POST" action="{{ route('catalogo.enlaces.eliminar', $enlace) }}" onsubmit="return confirm('¿Eliminar este enlace? Dejará de funcionar.')">
                                @csrf @method('DELETE')
